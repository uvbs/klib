...
#ifdef DEBUG
    if( cnt)printf("ac-bnfa: %d nfa optimizations found in %d states\n",cnt,bnfa->bnfaNumStates);
#endif
    return 0;
}
...
int _bnfa_conv_list_to_csparse_array(bnfa_struct_t * bnfa)
{
  ...
  */
  ps = (bnfa_state_t *) BNFA_MALLOC( nps*sizeof(bnfa_state_t),bnfa->nextstate_memory);
  if( !ps )
  {
      /* Fatal */
      return -1;
  ...
  */
  pi = (bnfa_state_t *) BNFA_MALLOC( bnfa->bnfaNumStates*sizeof(bnfa_state_t),bnfa->nextstate_memory);
  if( !pi )
  {
      /* Fatal */
      return -1;
...
void bnfa_print_qinfo(void)
{
    ...
    {
        printf("ac-bnfa: queue size     = %d, max = %d\n",snort_conf->max_inq, MAX_INQ );
        printf("ac-bnfa: queue flushes  = "STDu64"\n", snort_conf->tot_inq_flush );
        printf("ac-bnfa: queue inserts  = "STDu64"\n", snort_conf->tot_inq_inserts );
        printf("ac-bnfa: queue uinserts = "STDu64"\n", snort_conf->tot_inq_uinserts );
    }
#endif
}
static
