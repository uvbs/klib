...
#ifndef _SF_SERIALIZEDYNAMICARRAY_HPP_
#define _SF_SERIALIZEDYNAMICARRAY_HPP_
...
namespace SF {
    ...
    public:
        ...
        T *&get() { return pt_; }
        N &length() { return n_; }
        T &operator[](unsigned int idx) { SF_ASSERT( get() != NULL && 0 <= idx && idx < length() )(get())(idx)(length()); return *(get() + idx); }
    private:
    ...
    {
        const bool isFundamental = boost::is_fundamental<T>::value;
        if (isFundamental)
        {
...
#endif // ! _SF_SERIALIZEDYNAMICARRAY_HPP_
