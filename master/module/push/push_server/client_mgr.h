...
public:
    //----------------------------------------------------------------------
    // 往外发出的信号
    sigslot::signal1<client_info*>      s_on_client_online;     // 上线
    sigslot::signal1<client_info*>      s_on_client_offline;    // 下线
...
public:
    ...
    client_info* update_client_info(client_key& key, PT_CMD_ONLINE& cmd_online, BOOL& bNew);        ///< 更新客户端信息
...
protected:
    typedef std::map<client_key, client_info*>  mapClientIntoType;                  ///< 作一个hash并发访问的数组
