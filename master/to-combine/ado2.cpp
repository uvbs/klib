...
BOOL CJetEngine::CCompactDatabase(CString strDatabaseSource, CString strDatabaseDestination)
{
	try
	{
		...
		strDatabaseSource.ReleaseBuffer(0);
		strDatabaseDestination.ReleaseBuffer();
		::CoUninitialize();
		return hr == S_OK;
	}
	catch(_com_error &e) 
...
{	
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			val /= pow((double)10, (double)(vtFld.decVal.scale));
			break;
		case VT_UI1:
			val = vtFld.iVal;
			break;
...
BOOL CADOParameter::GetValue(double& dbValue)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			dblVal /= pow((double)10, (double)(vtVal.decVal.scale)); 
			break;
		case VT_UI1:
			dblVal = vtVal.iVal;
			break;
...
BOOL CADOParameter::GetValue(CString& strValue, CString strDateFormat)
{
	...
	try
	{
		...
		case VT_DECIMAL:
			...
			val /= pow((double)10, (double)(vtVal.decVal.scale)); 
			strVal = DblToStr(val);
			}
			break;
		case VT_DATE:
